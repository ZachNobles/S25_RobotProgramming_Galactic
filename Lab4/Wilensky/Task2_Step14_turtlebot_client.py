...
from rclpy.parameter import Parameter ### line added in Lab 4 Task 2
from rcl_interfaces.msg import ParameterDescriptor  ### line added in Lab 4 Task 2
...
class TurtleClient(Node):
    def __init__(self):
        ...
        ### Added during lab 4: create parameter for turtle color. Changes made by Joelle Wilensky
        ...
        ### added during lab 4: modyfing default color. Changes made by Joelle Wilenksy
        self.color_cli = self.create_client(SetColor, 'SetColor')
        while not self.color_cli.wait_for_service(timeout_sec=1.0):
            self.get_logger().info('Waiting for SetColor service...')
        self.color_req = SetColor.Request()
        self.color_req.color = turtleColor
        self.color_cli.call_async(self.color_req)
        self.server_call = True
        self.service_future = self.color_cli.call_async(self.color_req)
        
        ### Lab 4: Task 2, Step 14 parameter for pen color. Changes made by Joelle Wilensky
        self.declare_parameter('penSize', 3.0, ParameterDescriptor(description= 'Turtle pen size'))
        penSize = self.get_parameter('penSize').get_parameter_value().double_value
        self.get_parameter('penSize').get_parameter_value().double_value
...
def main(args=None):
    ...
    while rclpy.ok():
        ...
        ### cli_obj.twist_pub.publish(cmd_msg) # commented out publishing of command velocity in main func
